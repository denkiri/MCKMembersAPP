...
import com.deletech.mckmembersapp.repository.LoginRepository
import com.deletech.mckmembersapp.screens.login.state.LoginErrorState
...
import com.deletech.mckmembersapp.screens.login.state.emailEmptyErrorState
import com.deletech.mckmembersapp.screens.login.state.passwordEmptyErrorState
...
//        _profileResult.value=Resource.Idle()
    ...
    fun resetDefaultPassword(mobileNumber: String, password: String,defaultPassword:String) {
        viewModelScope.launch {
            _loginRequestResult.value = Resource.Loading()
            try {
                val loginResponse = repository.resetDefaultPassword(mobileNumber, password,defaultPassword)
                if (loginResponse is Resource.Success) {
                    _isLoading.value = false
                    _loginRequestResult.value = Resource.Success(loginResponse.data!!)
                } else {
